...
- [Giao dịch (Deal — GĐ2)](docs/features/deal.md) — giao dịch cọc→hợp đồng→hoàn tất (`deals`+`deal_payments`+`commissions`, xoá mềm): chuyển giai đoạn tự đổi `properties.status` (sold/rented/available), đợt thanh toán (đã thu/còn lại), hoa hồng tự tính theo % + đánh dấu chi (`commission_manage`); trang `/deals` (list+drawer) + nhóm cap `deal`.
- [Báo cáo (Report — GĐ2)](docs/features/report.md) — báo cáo read-only tổng hợp (không bảng mới): phễu chuyển đổi + nguồn khách + doanh số (deals) + hiệu suất nhân viên; `GET api/report` (data-scope) + xuất Excel (`Coordinate::stringFromColumnIndex`, tránh bug `$col++`); trang `/reports` (CSS bar, không thư viện chart) + nhóm cap `report`.
- [Multi-tenant theo PREFIX (GĐ4 — Bước 0)](docs/features/multi-tenant.md) — 1 deploy phục vụ N sàn, mỗi sàn 1 bộ bảng riêng cùng DB (prefix động), resolve theo path `domain.com/{key}/...`. Cô lập bằng `DB_PREFIX` + **thư mục cache riêng** (`path.cache` rebind — gồm cả `access_token_*` nên token cô lập, KHÔNG cần JWT claim/sửa vendor); resolver ở `index.php` (trước boot), bảng trung tâm `core_tenants` (prefix `core_`), `TenantProvisioner` + `api/central/*`, FE `utils/tenant.js` (baseURL/basename/`tstore` namespaced) gate `REACT_APP_MULTI_TENANT`. Opt-in, passthrough = tương thích ngược.
